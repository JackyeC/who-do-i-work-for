...
| **Forensic report** | `newsletter/outputs/forensic/*.md` | Same Edge **`publish-desk-publication`** with **`kind: "forensic"`** and **`published_to_site: true`** — live on **`/integrity-report`** (does not replace the bi-hourly desk). Script: **`scripts/content-engine/publish-forensic-publication.example.sh`**. RPCs: **`wdiwf_latest_forensic_publication`**, **`wdiwf_forensic_publications_recent`**. |
