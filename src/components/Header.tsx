...
function DropItem({ to, label, icon: Icon, desc }: { to: string; label: string; icon: React.ElementType; desc?: string }) {
  return (
    <Link to={to} className="flex items-start gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors hover:bg-accent group">
      ...
      <div className="flex flex-col">
        ...
        {desc && <span className="text-xs text-muted-foreground leading-snug mt-0.5">{desc}</span>}
      </div>
    </Link>
  );
}
...
function DropHeading({ children }: { children: React.ReactNode }) {
  return <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground px-3 pt-2.5 pb-1">{children}</p>;
}
...
export function Header() {
  ...
  return (
    <header
      ...
      style={{ boxShadow: "0 1px 0 hsl(38 72% 50% / 0.06), 0 4px 20px -4px hsl(var(--foreground) / 0.04)" }}
    >
      <div className="container mx-auto px-4 h-[4.5rem] flex items-center justify-between gap-4">
        ...
        <nav className="hidden lg:flex items-center gap-0.5 flex-1 justify-center">
          ...
          <Link
            ...
            className={cn(
              "flex items-center gap-1.5 text-sm font-medium px-3 py-2 rounded-lg transition-colors",
              location.pathname === "/" ? "text-foreground bg-accent" : "text-muted-foreground hover:text-foreground hover:bg-accent/50"
            )}
          >
          ...
          <div className="pt-2 border-t border-border/40 space-y-2">
            {user ? (
              ...
              <Link to="/login" onClick={() => setMobileOpen(false)}>
                <Button variant="default" size="default" className="w-full rounded-xl">Sign In</Button>
              </Link>
            )}
          </div>
        </div>
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        active ? "text-foreground bg-accent" : "text-muted-foreground hover:text-foreground active:bg-accent/50"
      )}
    >
