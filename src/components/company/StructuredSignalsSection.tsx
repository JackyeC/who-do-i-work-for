...
import { LOBBYING_FACTS_ONLY_NOTE } from "@/lib/lobbyingSignalExplainer";
...
interface StructuredSignalsProps {
  ...
  lastReviewed?: string;
  updatedAt?: string;
  /** Opens full LDA / state lobbying drawer on company profile */
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await (supabase as any)
        .from("issue_signals")
        .select("id, description, amount, source_url, confidence_score, signal_type")
        ...
        {lobbyingIssueRows.length > 0 && (
          <div className="py-4 border-b border-border/30 last:border-b-0">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-sm font-semibold text-foreground">Lobbying filings (public record)</p>
            </div>
            <p className="text-[0.65rem] font-mono uppercase tracking-wider text-muted-foreground mb-2">
              {LOBBYING_FACTS_ONLY_NOTE}
            </p>
            <p className="text-xs text-muted-foreground mb-3">
              Rows from lobbying-related issue signals. Expand for topic parsing and filing definitions—verify in primary sources.
            </p>
                    ...
                    <CollapsibleContent className="px-3 pb-3 border-t border-border/30">
                      {row.amount != null && row.amount > 0 && (
                        <p className="text-xs font-mono font-semibold text-foreground pt-2 mb-2">
                          ...
                          <span className="text-muted-foreground font-sans font-normal">as cited in source summary</span>
                        </p>
                      )}
            ...
            {props.onLobbyingDetailClick && (
              <Button type="button" variant="outline" size="sm" className="mt-3 w-full sm:w-auto" onClick={props.onLobbyingDetailClick}>
                Open lobbying detail (issues, states, linkages)
              </Button>
            )}
          </div>
        )}
