  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      const [apps, alerts, values, growth] = await Promise.all([
        ...
        supabase.from("user_signal_alerts").select("id", { count: "exact", head: true }).eq("user_id", user!.id).eq("is_read", false),
        supabase.from("user_values_profile").select("id").eq("user_id", user!.id).maybeSingle(),
        supabase.from("employee_growth_tracker").select("id", { count: "exact", head: true }).eq("user_id", user!.id),
      ]);
      return {
