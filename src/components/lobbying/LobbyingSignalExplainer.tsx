...
export function LobbyingSignalExplainer({
  ...
  return (
    <div className={cn("space-y-3 text-xs text-muted-foreground leading-relaxed", className)}>
      {showTopics ? (
        <div>
          <p className="font-semibold text-foreground mb-1.5">Topics reflected in disclosures</p>
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-1 items-center">
        <span className="font-mono text-[0.65rem] uppercase tracking-wider text-muted-foreground">Receipts</span>
        {validReceipt ? (
          <a
            href={sourceUrl!}
            target="_blank"
