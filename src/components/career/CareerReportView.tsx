    ...
    <div className="space-y-4">
      ...
      <div ref={reportRef} className="space-y-4 bg-card rounded-xl p-6 border border-border">
        ...
        <div className="text-center pb-4 border-b border-border">
          <p className="text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Career Roadmap Report</p>
        ...
        <ReportSection icon={Target} title="Gap Analysis" number={2}>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1.5">Your Matching Skills</p>
              <div className="flex flex-wrap gap-1">
                {(track.completed_skills || []).slice(0, 8).map((s: string, i: number) => (
                  <span key={i} className="text-[10px] bg-[hsl(var(--civic-green))]/10 text-[hsl(var(--civic-green))] rounded px-1.5 py-0.5">
                    ✓ {s}
                  </span>
                ))}
              </div>
            ...
            <div>
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1.5">Skills to Build</p>
              <div className="flex flex-wrap gap-1">
        ...
        <div className="text-center pt-3 border-t border-border">
          <p className="text-[9px] text-muted-foreground italic">
            Generated by CivicLens Career Intelligence · {new Date().toLocaleDateString()} · AI-powered estimates, not guarantees
          </p>
        </div>
      </div>
    </div>
...
function SalaryBar({ label, low, high, height, highlight }: {
  ...
  return (
    <div className="flex-1 text-center">
      ...
      <p className="text-[10px] font-medium text-foreground mt-1 truncate">{label}</p>
