...
import {
  Search, ArrowRight, Shield, Users, Eye, DollarSign, Landmark, HeartHandshake,
  Share2, Twitter, Linkedin, Link2, Check, MessageCircle, CircleHelp, Target,
...
function coverageLabel(score: number) {
  if (score >= 70) return { level: "High", color: "text-civic-green", desc: "Multiple verified public sources available." };
  if (score >= 40) return { level: "Medium", color: "text-civic-yellow", desc: "Some public evidence exists, but gaps remain." };
  return { level: "Low", color: "text-civic-red", desc: "Limited public data. Score may not fully reflect employer quality." };
}
...
const DEMO_SIGNALS = [
  {
    ...
    subscore: 72,
    status: "Stable",
    explanation: "No WARN Act layoff notices filed in the past 12 months. Headcount trend appears steady based on public filings.",
  },
  {
    key: "hiring",
    ...
    subscore: 38,
    status: "Low",
    explanation: "AI screening tools detected in the hiring process. No published bias audit or candidate disclosure found.",
  },
  {
    key: "compensation",
    ...
    subscore: 55,
    status: "Partial",
    explanation: "Some salary ranges posted on job listings. No public pay equity report available. Median offer sits below BLS benchmark for this industry.",
  },
  {
    key: "influence",
    label: "Influence Exposure",
    icon: Landmark,
    weight: 15,
    subscore: 28,
    status: "Significant",
    explanation: "$5.2M in lobbying spend. Corporate PAC active across 14 congressional districts. Multiple dark money channels flagged.",
  },
  {
    key: "leadership",
    label: "Leadership & Culture Trust",
    icon: HeartHandshake,
    weight: 20,
    subscore: 50,
    status: "Mixed",
    explanation: "Executive team publicly visible. Glassdoor sentiment is polarized. No published DEI accountability metrics.",
  },
];

const DEMO_COVERAGE = 58; // data coverage %

function computeClarityScore(signals: typeof DEMO_SIGNALS) {
  return Math.round(signals.reduce((sum, s) => sum + (s.subscore * s.weight) / 100, 0));
}

const DEMO_QUESTIONS = [
  "Can you share the bias audit for any AI tools used in your hiring process?",
  "What is the salary band for this role, and how is it benchmarked?",
  "Has the company filed any WARN Act notices in the past 24 months?",
  "What percentage of leadership roles are held by underrepresented groups?",
  "Can you explain the company's lobbying priorities and how they affect employees?",
];
    ...
    <div className="bg-card border border-border p-5 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <signal.icon className="w-4.5 h-4.5 text-primary" strokeWidth={1.5} />
      ...
      <div className={`inline-self-start w-fit px-2 py-0.5 text-[0.65rem] font-semibold uppercase tracking-wider ${band.color} ${band.bg} border ${band.border}`}>
        {signal.status}
      </div>
...
export default function WouldYouWorkHere() {
  ...
  const clarityScore = computeClarityScore(DEMO_SIGNALS);
  const coverage = coverageLabel(DEMO_COVERAGE);
      ...
      <section className="px-6 lg:px-16 pt-20 pb-12 lg:pt-28 lg:pb-16 text-center max-w-[800px] mx-auto">
        ...
        <div className="flex max-w-[500px] mx-auto border border-border bg-card">
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none outline-none py-3.5 text-foreground font-sans text-[15px] placeholder:text-muted-foreground"
          />
          <button
            onClick={handleSearch}
            className="bg-primary text-primary-foreground px-5 font-mono text-[0.7rem] tracking-wider uppercase font-semibold hover:brightness-110 transition-all"
          >
            Scan
          </button>
        </div>
      ...
      <section className="px-6 lg:px-16 pb-8 max-w-[840px] mx-auto w-full">
        <div className="bg-card border border-border relative">
          ...
          <div className="p-8 pb-6 flex flex-col sm:flex-row items-center gap-8 border-b border-border">
            <ScoreRing score={clarityScore} />
            <div className="flex-1 text-center sm:text-left">
              <div className="font-mono text-[0.65rem] uppercase tracking-widest text-muted-foreground mb-1.5">Data Coverage</div>
              <div className={`text-2xl font-bold ${coverage.color} mb-1`}>{coverage.level}</div>
              <p className="text-sm text-muted-foreground leading-relaxed max-w-[340px]">{coverage.desc}</p>
          ...
          {/* Jackye's Verdict */}
          <div className="mx-6 mb-6 p-5 border-l-2 border-primary bg-primary/[0.04]">
            ...
            <div className="inline-block px-2.5 py-1 text-xs font-semibold uppercase tracking-wider text-civic-yellow bg-civic-yellow/10 border border-civic-yellow/30 mb-3">
              Proceed with Caution
            </div>
            <p className="text-sm text-foreground leading-relaxed mb-3">
              This company has significant political exposure and undisclosed AI hiring tools. The offer itself isn't bad — but the salary sits below market and the hiring process lacks transparency. You have leverage here. Use it. Ask about the AI screening before your next interview, and request the pay band in writing.
            </p>
      ...
      <section className="border-t border-border px-6 lg:px-16 py-16 text-center">
        ...
        <div className="flex gap-3 justify-center flex-wrap">
          <button
            ...
            className="bg-primary text-primary-foreground px-7 py-3 font-mono text-[0.7rem] font-semibold tracking-wider uppercase hover:brightness-110 transition-all flex items-center gap-2"
          >
            Run Employer Scan <ArrowRight className="w-4 h-4" />
          </button>
          <button
            onClick={() => navigate("/check")}
            className="border border-border text-muted-foreground px-7 py-3 font-mono text-[0.7rem] tracking-wider uppercase hover:border-primary hover:text-primary transition-all"
          >
            Upload an Offer
          </button>
          <button
            onClick={() => navigate("/ask-jackye")}
            className="border border-border text-muted-foreground px-7 py-3 font-mono text-[0.7rem] tracking-wider uppercase hover:border-primary hover:text-primary transition-all flex items-center gap-2"
          >
